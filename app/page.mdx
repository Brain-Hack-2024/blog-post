...
The acoustoelectric effect (AE) allows us to precisely target tiny areas—as small as a millimeter—and use focused ultrasound to single out the electric field of a region. Our goal is to use it to make brain-computer interfaces (BCI's) with 27,000x higher resolution.
...
Evidently, neurons don't normally "sing" at unique frequencies. But perhaps with external pertubation, we can make them do so, i.e. with ultrasound.
</div>
